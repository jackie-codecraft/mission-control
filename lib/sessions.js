...
function getDailyStats() {
  const { modelCostPer1M } = require('./events');
  const sessionsJson = loadSessionsJson();
  const now = new Date();
  ...
  for (const [key, v] of Object.entries(sessionsJson)) {
    ...
    const rate = modelCostPer1M(v.model || v.modelOverride || '');
    const cost = (tokens / 1_000_000) * rate;

    if (updated >= todayMs) { tokensToday += tokens; costToday += cost; }
    if (updated >= weekMs) { tokensWeek += tokens; costWeek += cost; }
    if (updated >= monthMs) { tokensMonth += tokens; costMonth += cost; }

    if (updated >= todayMs && key.includes(':subagent:')) {
      subagentsToday++;
    }
  }

  return { subagentsToday, tokensToday, tokensWeek, tokensMonth, costToday, costWeek, costMonth };
}

module.exports = { getSessions, parseSessions, getDailyStats, loadSessionsJson, extractTaskBrief };
