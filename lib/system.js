...
function getDiskUsage() {
  try {
    const raw = execSync('df -BM / 2>/dev/null', { encoding: 'utf8', timeout: 5000 }).trim();
    const lines = raw.split('\n');
    for (const line of lines.slice(1)) {
      const parts = line.trim().split(/\s+/);
      if (parts.length >= 5) {
        const size = parseInt(parts[1]) || 0;   // MB
        const used = parseInt(parts[2]) || 0;   // MB
        const avail = parseInt(parts[3]) || 0;  // MB
...
module.exports = { parseUptime, parseCpu, parseMemory, getGatewayStatus, getDiskUsage };
