...
function getGatewayStatus() {
  try {
    ...
    const isRunning = result.toLowerCase().includes('running') || result.toLowerCase().includes('active');
    return { status: isRunning ? 'running' : 'stopped', raw: result.trim() };
  } catch (e) {
    return { status: 'unknown', raw: e.message };
  }
}
